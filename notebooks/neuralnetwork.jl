...
# ╔═╡ d411a956-64ca-11f0-203d-15bae83ab8c3
begin
    using DataFrames
    using CSV
    using Flux
...
# ╔═╡ a12adcb6-f137-408b-823c-1bc0234de9fc
begin
    comminution_df = CSV.read("comminution.csv", DataFrame)
    flotation_df = CSV.read("flotation.csv", DataFrame)
    drillholes_df = CSV.read("drillholes.csv", DataFrame)
    
    md"**CSV files loaded**"
end


# ╔═╡ bb3335c4-f912-4db0-96b9-b08c0c23f369
# --- Cell 3: Combining ALL Data and Preprocessing (Definitive Version) ---
begin
    # STEP 1: Ensure that DataFrames have the same size
    min_rows = min(nrow(comminution_df), nrow(flotation_df), nrow(drillholes_df))

    comminution_sliced = comminution_df[1:min_rows, :]
    flotation_sliced = flotation_df[1:min_rows, :]
    drillholes_sliced = drillholes_df[1:min_rows, :]

    # STEP 2: Rename columns to avoid duplicates
    rename!(flotation_sliced, Dict(col => string(col) * "_flotation" for col in names(flotation_sliced)))
    rename!(drillholes_sliced, Dict(col => string(col) * "_drillholes" for col in names(drillholes_sliced)))

    # Joining the three DataFrames
    combined_df = hcat(comminution_sliced, flotation_sliced, drillholes_sliced)

    # STEP 3: Define Features and Targets from the COMBINED DataFrame
    
    # Our targets (what we want to predict)
    targets = ["Cu ppm_flotation", "Au ppm_flotation", "Ag ppm_flotation", "Fe ppm_flotation", "S ppm_flotation"]

    # Our features (inputs) are ALL other columns, except the targets
    # and HOLEID, which is not numeric.
    features = setdiff(names(combined_df), vcat(targets, ["HOLEID_drillholes"]))

    # STEP 4: Clean and Convert the Data
    data_clean = dropmissing(combined_df, vcat(features, targets))

    features_array = Matrix(data_clean[:, features])
    targets_array = Matrix(data_clean[:, targets])

    # STEP 5: Stable Normalization
    epsilon = 1e-8 
    features_mean = mean(features_array, dims=1)
    features_std = std(features_array, dims=1)
    features_normalized = (features_array .- features_mean) ./ (features_std .+ epsilon)

    targets_mean = mean(targets_array, dims=1)
    targets_std = std(targets_array, dims=1)
    targets_normalized = (targets_array .- targets_mean) ./ (targets_std .+ epsilon)

    # STEP 6: Prepare data for Flux and ensure Float32 type
    X = permutedims(features_normalized)
    Y = permutedims(targets_normalized)
...
# ╔═╡ 35a980f1-7264-4bd3-87c6-a306ba9ac57d
#Defining the Neural Network 
begin
    output_size = length(targets)
    ...
    md"**Neural Network architecture defined.**"
end


# ╔═╡ 2e068abe-ac10-48b5-884f-fb60b017cd10
begin
    #predictions for data
    predictions_normalized = model_multi(X)
    
    #transforming the results to the original scale
    predictions_desnormalized = (predictions_normalized' .* targets_std) .+ targets_mean
    
    #creating a plot for each mineral
    plots_list = []
    for i in 1:length(targets)
        p = plot(1:size(targets_array, 1), 
                 [targets_array[:, i], predictions_desnormalized[:, i]],
                 title=targets[i],
    ...
    #plots
    plot(plots_list..., layout=(length(targets), 1), size=(800, 1000))
end


# ╔═╡ 43f5be29-87a8-4dc4-a154-46fb26a2647d
#model training
begin
    #the loss function remains the same
    loss_multi(m, x, y) = Flux.mse(m(x), y)
    

    #decrease the learning rate to 0.0001 to avoid instability 
    optimizer = ADAM(0.0001) 


    #initializes the optimizer state and avoids the warning.
    opt_state = Flux.setup(optimizer, model_multi)
    
    #training loop
    for epoch in 1:200
        Flux.train!(loss_multi, model_multi, [(X, Y)], opt_state)
    end
    
    md"**Training completed.**"
end
...
manifest_format = "2.0"
project_hash = "36dcdc689171b4431f04c8e6c7c3007b7977eec0"
...
git-tree-sha1 = "209390b236bb04b289c708054fe1df06a4b314b5"
