...
# ╔═╡ 3e150110-54f5-11f0-1641-7b1a13784448
using Plots, GLM, DataFrames

# ╔═╡ ba4dc76d-740e-4bf0-85fa-7458127f6cf9
begin

	# Create a DataFrame with x from 1 to 10 and y values with added random noise
	df = DataFrame(x = 1:10, y = [5, 8, 10, 12, 14, 16, 18, 20, 26, 30] .+ randn(10))
	
	# Scatter plot
	scatter(df.x, df.y, label = "Data Points", legend = :topleft, xlabel = "x", ylabel = "y", title = "Linear Regression Fit")
	
	# Fit a linear model (y ~ x)
	model = lm(@formula(y ~ x), df)
	
	# Extract intercept (a) and slope (b)
	a, b = coef(model)
	
	# Plot the regression line
	plot!(df.x, a .+ b .* df.x, 
	      label = "Fit: y = $(round(a, digits=2)) + $(round(b, digits=2))x", 
	      linewidth = 2, 
	      color = :red)
...
manifest_format = "2.0"
project_hash = "f22923b06cf277e42b32623ac9cd56050d286ed9"
